...
module.exports = {
    ...
    checkResErr: async (res, throwIt = true) => {
        ...
        if (errAPI) {
            ...
            // 特别处理403权限错误
            if (err.status === 403 && errAPI.type === 'permission_error') {
                console.log('[31mCookie权限错误![0m 服务器返回：' + errAPI.message);
                console.log('[33m您的Cookie可能已过期，请更新Cookie后重试[0m');
                err.isCookieError = true;
            }
